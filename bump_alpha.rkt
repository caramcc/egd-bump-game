...
;;;;; CONSTANTS ;;;;;
(define ENEMY-IMG (circle 25 'solid 'black))
(define PLAYER-IMG (circle 25 'solid 'blue))
(define SCN (empty-scene 700 200))
(define MOV-CONST 25)
(define FRONT-DOOR (make-posn 50 0))
(define BACK-DOOR (make-posn 650 0))

(define TEST-ENEMIES (list (make-enemy (make-posn 50 75) 3)
                           (make-enemy (make-posn 300 100) 4)
                           (make-enemy FRONT-DOOR 1)
                           (make-enemy BACK-DOOR 5)))
(define TEST-PLAYER (make-player FRONT-DOOR 0 0))
